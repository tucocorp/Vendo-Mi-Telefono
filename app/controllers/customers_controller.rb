class CustomersController < ApplicationController
  ...
  # GET /customers.json
  def index
    @search = Customer.search(name_or_last_name_or_rut_or_email_or_address_or_phone_cont: params[:q])
    #raise @search.isnpect
    @customers = @search.result(distinct: true).page(params[:page]).per(5)
  end
